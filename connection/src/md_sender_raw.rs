...
unsafe extern "C" fn md_callback (_a: *mut libc::c_void, _app_handle : TRDP_APP_SESSION_T, p_msg: *const TRDP_MD_INFO_T, p_data: *mut u8, data_size :u32)
{
    ...
    unsafe {
        ...
        loop {
            ...
            if count < 2
            {
                ...
                let request_msg = CString::new("How are you?").unwrap().into_bytes_with_nul();
                let result = tlm_request(app_handle,
                    ...
                    delay, //delay
                    ptr::null_mut(),
                    request_msg.as_ptr() as *const u8,
                    request_msg.len() as u32,
                    ptr::null_mut(), 
                    ptr::null_mut());
